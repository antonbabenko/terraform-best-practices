# Інфраструктура сярэдняга памеру з Terraform

Крыніца: [https://github.com/antonbabenko/terraform-best-practices/tree/master/examples/medium-terraform](https://github.com/antonbabenko/terraform-best-practices/tree/master/examples/medium-terraform)

Гэты прыклад змяшчае код у якасці прыкладу структуравання канфігурацый Terraform для інфраструктуры сярэдняга памеру, якая выкарыстоўвае:

* 2 уліковыя запісы AWS
* 2 асобныя асяроддзі (`prod` і `stage` якія нічога не падзяляюць). Кожнае асяроддзе знаходзіцца ў асобным уліковым запісе AWS
...
- Добра, калі рэсурсы інфраструктуры ў кожным асяроддзі наўмысна адрозніваюцца і не могуць быць абагульненымі (напрыклад, некаторыя рэсурсы адсутнічаюць у адным асяроддзі або ў некаторых рэгіёнах)
{% endhint %}
