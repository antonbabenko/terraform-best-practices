...
1. 리소스 모듈의 바퀴를 다시 만들지 마세요<mark style="background-color:red;">(Don't reinvent the wheel! 이미 만들어져 잘 쓰이고 있는 것을 두고 굳이 비슷한 수준의 것을 새로 만드는 데 에너지를 쏟는 것을 피하라는 말)</mark>. 작업 중인 리소스에 대한 "인수 참조" 섹션에 정의된 대로 변수에 대한 `name`, `description`, 및 `default`값을 사용하세요.
