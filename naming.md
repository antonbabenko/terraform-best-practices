...
1. Only use lowercase letters and numbers.
...
7. When using conditions in an argument`count` / `for_each` prefer boolean values over using `length` or other expressions.
...
4. Keys in variable block in order: `description` , `type`, `default`, `validation`.
...
Make outputs consistent and understandable outside of its scope (when user is using a module it should be obvious what type and attribute of the value is returned).
...
7. Prefer `try()` (available since Terraform 0.13) over `element(concat(...))`
