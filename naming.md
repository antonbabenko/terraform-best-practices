...
10. 二重否定を避けてください：混乱を防ぐため、肯定的な変数名を使用してください。例えば、`encryption_disabled`ではなく`encryption_enabled`を使用してください。
11. `null`にすべきでない変数には`nullable = false`を設定してください。そうすれば、`null`を渡した場合にデフォルト値が使用されるようになります。`null`が許容される値の場合は、nullableを省略するか`true`に設定してください。
